...
# Add tanbeta consistency to chi vector to help lmfit minimization methods 
...
def calcChiSqrd(pspoint) : 
 ...
 chisqrd = chi[0]**2 + chi[1]**2 + chi[2]**2 + chi[3]**2
...
def runSVM(params, out4lmfit = True) : # 14 parameters 
 ...
 pspoint = runSPheno() 
 if not checkSPout(pspoint) : return np.full(4, 1E+50)
 
 pspoint += runVevacious() 
 if not checkVEVout(pspoint) : return np.full(4, 1E+50) 
 
 pspoint += runMicrOmegas()
 if not checkMicrout(pspoint) : return np.full(4, 1E+50) 
