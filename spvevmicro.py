...
class ScanVar : 
 ...
 tberr = 1 # Check v2/v1 == TanBeta (consistency between Vevacious results and input)
...
#------------------Functions---------------------
def makeLesHouchesin(point, exp_step = False) : # point is an lmfit.parameter.Parameter object with 14 parameters 
 '''
...
def runMicrOmegas() : 
 ...
 try : 
     ...
     print("STDOUT: " + str(migaspr.stdout,'utf-8'))
...
def checkSPout(point) : 
 ...
 if ScanVar.Hphystag == 1 : 
     if not abs(Mhh2 - Mhh1) >= 50 : return False # Exotic Higgs masses greater that 175 GeV 
 elif ScanVar.Hphystag == 2 : 
     if not 85 <= Mhh1 <= 105 : return False 
 else : return False
 if not (MAh2 - Mhh1) >= 50 : return False 
 if not (MHm2 - Mhh1) >= 50 : return False 
 #if not Mss >= 100 : return False # DM mass greater than 100 GeV 
...
def calcChiSqrd(pspoint) : 
 ...
 chisqrd = np.inner(chi,chi) # chi[0]**2 + chi[1]**2 + chi[2]**2
